...
async def get_skills():
    try:
        ...
        for leader in leadership_data:
            ...
            if skills is None:  # Skip if skills is None
                continue
            for skill in skills:
    ...
    try:
        ...
        if session_data:
            ...
            for leadership in leadership_data:
                leadership_skills = leadership.get('skills', [])
                if leadership_skills is None:  # Skip if skills is None
                    continue
                leadership_skills_dict = dict(leadership_skills)
                if all(skill in leadership_skills_dict and leadership_skills_dict[skill] >= int(value) for skill, value in session_skills.items()):
        ...
        for leader in filtered_leadership_data:
            if session_data:
                ...
                leader_skills = {skill[0]: skill[1] for skill in leader.get('skills', [])}
                if leader_skills:  # Ensure leader_skills is not None
        ...
        for leader in filtered_leadership_data:
            ...
            leader['potential'] = int(leader['potential'] * 100)  # Convert to percentage
