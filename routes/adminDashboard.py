    ...
    try:
        ...
        for leader in filtered_leadership_data:
            if session_data:
                ...
                # Check if there are non-zero skills to avoid division by zero
                if M_values:
                    # Calculate weights
                    ...
                    # Add competency score to leader data, rounded to three decimal points
                    leader['competency'] = round(competency_score, 3)
                else:
                    leader['competency'] = 0.0  # Set to 0.0 if no non-zero skills
            else:
                # If there are no non-zero skills, set competency to 0
                leader['competency'] = 0.0
        
        for leader in filtered_leadership_data:
            if 'potential' not in leader or leader['potential'] is None:
                leader['potential'] = 0.0
            leader['potential'] = round(leader['potential'] * 100, 2)  # Convert to percentage and round to two decimal points
